		...
		<center>
			...
			<p>
				<form action="" name="searchForm" method="post" onclick="checkInf()">
					输入查询的条件： <select name="tiaojian">
										<option value="mohu">模糊查询</option>
										<option value="key">关键字查询</option>
									</select>
					&nbsp;&nbsp;
					<select name="guanjian" style="DISPLAY:none">
						<option value="cargo.Cargo_ID">商品编号</option>
						<option value="Cargo_Name">商品名</option>
						<option value="Cargo_Type">商品类别</option>
						<option value="Sell_Num">数量</option>
						<option value="Sell_Price">单价</option>
					</select>
					<input type="text" name="keyword">
					&nbsp;&nbsp;
					<input type="submit" value="查询" onclick="checkInput()">
					&nbsp;&nbsp;
			...
			<table align="center" width="400" >
				...
				<c:forEach var="cargo" items="${array}">
					<tr class="cargotr">
						...
						<td>${cargo.sell_num}个</td>
						<td>${cargo.sell_price}￥</td>
						<td><a href="DeleteCargo?id=${cargo.cargo_id }">删除</a></td>
						<td><a href="EidtCargo?id=${cargo.cargo_id }">修改</a></td>
					</tr>
				</c:forEach>
			</table>
		</center>
